...
class Wav2VecEncoder(FairseqEncoder):
    def __init__(self, cfg: Wav2Vec2AsrConfig, output_size=None):
        ...
        if output_size is not None:      # mq227: cfg.decoder_embed_dim, it is set when calling Wav2VecEncoder, set to None to keep 768
            targ_d = output_size
    ...
    def forward(self, source, padding_mask, **kwargs):
        ...
        # mq227: self.proj: Linear(in_features=768, out_features=48, bias=True)
        if self.proj:
            x = self.proj(x)   # mq227: 342 x 2 x 768 -> 342 x 2 x 48
